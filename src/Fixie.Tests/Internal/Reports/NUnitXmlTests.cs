namespace Fixie.Tests.Internal.Reports
{
    ...
    public class NUnitXmlTests : MessagingTests
    {
        ...
        static void XsdValidate(XDocument doc)
        {
            ...
            using (var xmlReader = XmlReader.Create(Path.Combine("Internal", Path.Combine("Reports", "NUnitXmlReport.xsd"))))
            {
        ...
        string ExpectedReport
        {
            get
            {
                ...
                return XDocument.Parse(File.ReadAllText(Path.Combine("Internal", Path.Combine("Reports", "NUnitXmlReport.xml"))))
