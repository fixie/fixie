namespace Fixie.Tests.Internal.Reports
{
    using System;
    using System.IO;
    ...
    public class XUnitXmlTests : MessagingTests
    {
        ...
        static void XsdValidate(XDocument doc)
        {
            ...
            using (var xmlReader = XmlReader.Create(Path.Combine("Internal", Path.Combine("Reports", "XUnitXmlReport.xsd"))))
            {
        ...
        string ExpectedReport
        {
            get
            {
                ...
                return XDocument.Parse(File.ReadAllText(Path.Combine("Internal", Path.Combine("Reports", "XUnitXmlReport.xml"))))
